...
public class KeystoreTest {
    ...
    public static final String MYSTORE = "myStore.pkcs12";

    @Test
    public void createKeyStore() throws GeneralSecurityException, IOException, OperatorCreationException {
        if (Files.exists(Path.of(MYSTORE))) {
            Files.delete(Path.of(MYSTORE));
        }
        ...
        keyStore.store(new FileOutputStream(MYSTORE), "password".toCharArray());
    ...
    public X509Certificate generateRootCert(KeyPair pair)
...
//        certificateBuilder.addExtension(org.bouncycastle.asn1.x509.Extension.subjectAlternativeName, false, subjectAltNames);
        ...
        final AsymmetricKeyParameter keyParameter = PrivateKeyFactory.createKey(pair.getPrivate().getEncoded());
        final ContentSigner signer = signerBuilder.build(keyParameter);
